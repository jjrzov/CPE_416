...
class ObstacleDetectorImprovedNode(Node):
    ...
    def process_scan(self,):
        ...
        # self.get_logger().info("FOUND AN OBJECT: %d", % distance)
        ...
        if distance != 0:
            ...
            laser2object_msg.child_frame_id = 'deteced_obstacle' # Set correct child frame
            
            laser2object_msg.transform.translation.x = distance # translation of distance in x

            try:
                # Lookup transform
                odom2laser = self.tf_buffer.lookup_transform('diff_drive/odom', self.latest_laser.header.frame_id , rclpy.time.Time())
            ...
            obj2odom_msg = self.multiply_transforms(odom2laser, laser2object_msg) # Set correct parent frame
            obj2odom_msg.header.frame_id = 'diff_drive/odom'
            obj2odom_msg.child_frame_id = 'detected_obstacle' # Set correct child frame

            # Call the broadcaster
            self.tf_broadcaster.sendTransform(obj2odom_msg)
